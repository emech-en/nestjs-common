...
const destPath = path.join(packagePath, './dest');
...
async function typescriptCopy({ from, to }) {
  ...
  const files = glob.sync('**/*.{d.ts,js}', { cwd: from });
...
async function run() {
  try {
    ...
    // TypeScript
    await typescriptCopy({ from: destPath, to: buildPath });

    await createModulePackages({ from: destPath, to: buildPath });
  } catch (err) {
    console.error(err);
    process.exit(1);
  }
